...
def createMaster():
    ...
    for l in Pattern.lenOpts:
        constraints[l] = LpConstraintVar( "Min" + str( l ), DVarConstraintGE, rollDemand[l] )
        prob += constraints[l]

    # The surplus variables are created
    surplusVars = []
    for i in Pattern.lenOpts:
        surplusVars += [DVar( "Surplus " + i, 0, None, DVarC, -surplusPrice[i] * obj - constraints[i] )]
...
def addPatterns( obj, constraints, newPatterns ):
    ...
    pattVars = []
    for i in Patterns:
        pattVars += [DVar( "Pattern " + i.name, 0, None, DVarC, ( i.cost - Pattern.trimValue * i.trim() ) * obj\
